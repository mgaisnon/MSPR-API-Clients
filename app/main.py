...
def read_customer(
    customer_id: int, db: Session = Depends(get_db), _=Depends(verify_api_key)
):
...
@app.post(
    "/customers/", response_model=schemas.Customer, status_code=status.HTTP_201_CREATED
)
def create_customer(
...
def update_customer(
    ...
    if customer.username != db_customer.username:
        existing_customer = crud.get_customer_by_username(
            db, username=customer.username
        )
